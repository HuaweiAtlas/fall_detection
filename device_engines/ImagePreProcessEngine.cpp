/**
* @file ImagePreProcessEngine.cpp
*
...
ImagePreProcessEngine::~ImagePreProcessEngine()
{
    ...
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[ImagePreProcessEngine]ImagePreProcessEngine Engine Destory");
}

HIAI_StatusT ImagePreProcessEngine::Init(const hiai::AIConfig& config, const std::vector<hiai::AIModelDescription>& modelDesc)
{
    if (NULL == dvppConfig_) {
        dvppConfig_ = std::make_shared<DvppConfig>();
        if (NULL == dvppConfig_ || NULL == dvppConfig_.get()) {
            HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[ImagePreProcessEngine] call dvppConfig_ make_shared failed");
            return HIAI_ERROR;
        }
    }

    //get config from ImagePreProcessEngine Property setting of user.
    std::stringstream ss;
    ...
    if (DVPP_SUCCESS != CreateDvppApi(pidvppapi_)) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[ImagePreProcessEngine]Create DVPP pidvppapi_ fail");
        return HIAI_ERROR;
    }
...
//  3. DVPP_CTL_VPC_PROC
int ImagePreProcessEngine::HandleJpeg(const ImageData<u_int8_t> &img)
{
    if (NULL == pidvppapi_) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[ImagePreProcessEngine] pidvppapi_ is null!\n");
        return HIAI_ERROR;
    }
    ...
    if (0 != DvppCtl(pidvppapi_, DVPP_CTL_JPEGD_PROC, &dvppApiCtlMsg)) { //if this single jpeg pic is processed with error, return directly, and then process next pic if there any.
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[ImagePreProcessEngine] JPEGDERROR, FrameID:%u", imageFrameID_);
...
int ImagePreProcessEngine::UpdateCropPara(const Rectangle<Point2D> &rect)
{
    ...
    HIAI_ENGINE_LOG("[ImagePreProcessEngine]: %d, %d, %d, %d", (*dvppConfig_).point_x, (*dvppConfig_).point_y,(*dvppConfig_).crop_width,(*dvppConfig_).crop_height);

    if ((*dvppConfig_).point_x < 0
        || (*dvppConfig_).point_y < 0
        || (*dvppConfig_).crop_height <= 0
        || (*dvppConfig_).crop_width <= 0) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[ImagePreProcessEngine]Some crop parms are invalid!");
        return HIAI_ERROR;
    }
    return HIAI_OK;
}

bool ImagePreProcessEngine::SendPreProcessData()
{
    if (NULL == dvppOut_) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[ImagePreProcessEngine]Nothing to send!");
        return false;
    }
    ...
    if (dvppOut_->v_img.size() > 0) {
        ...
        do {
            ...
            if (HIAI_QUEUE_FULL == hiaiRet) {
                HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[ImagePreProcessEngine] queue full, sleep 200ms");
                usleep(SEND_DATA_INTERVAL_MS);
            }
        } while (hiaiRet == HIAI_QUEUE_FULL);

        if (HIAI_OK != hiaiRet) {
            HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[ImagePreProcessEngine] SendData failed! error code: %d", hiaiRet);
            return false;
        }
    }
    return true;
...
void ImagePreProcessEngine::ClearData()
{
    dvppIn_ = NULL;
    dvppOut_ = NULL;
}
...
bool ImagePreProcessEngine::ProcessCrop(VpcUserCropConfigure &area, const int &width, const int &height, const int &realWidth, const int &realHeight)
{
    //default no crop
    ...
    //user crop
    if (NeedCrop()) {
        HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[ImagePreProcessEngine] User crop, System crop");
        ...
        HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[ImagePreProcessEngine] leftOffset: %u, rightOffset: %u, upOffset: %u, downOffset: %u",
        ...
        const ImageData<u_int8_t> &img, const FILE_TYPE &type, const int &format)
{
    ...
    HIAI_ENGINE_LOG("[ImagePreProcessEngine] realWidth: %d, realHeight: %d, width: %d, height: %d", realWidth, realHeight, width, height);
    ...
    if (0 == resizeWidth || 0 == resizeHeight) {
        HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[ImagePreProcessEngine] user donnot need resize, resize width/height use real size of pic");
    ...
    if (resizeWidth > DVPP_SUPPORT_MAX_WIDTH || resizeWidth < DVPP_SUPPORT_MIN_WIDTH
        ...
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[ImagePreProcessEngine]resize range error, resizeWidth:%u, resizeHeight:%u", resizeWidth, resizeHeight);
        return HIAI_ERROR;
    }
    ...
    if (ProcessCrop(inputConfigure->cropArea, width, height, realWidth, realHeight)) {
        ...
        if (nullptr == outputConfigure->addr) {
            HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[ImagePreProcessEngine]outputConfigure->addr is null");
            return HIAI_ERROR;
        }
        HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[ImagePreProcessEngine]inputConfigure cropArea:%u, %u, %u, %u, outputConfigure outputArea:%u, %u, %u, %u, stride:%u, %u, %u",
        ...
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[ImagePreProcessEngine]ProcessCrop error");
        return HIAI_ERROR;
    }
    ...
    if (!dvppConfig_->dvpp_para.empty())
    {
        ...
        HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[ImagePreProcessEngine_1]dvpp_para:%s", dvppConfig_->dvpp_para.c_str());
    }
    ...
    if(0 != DvppCtl(pidvppapi_, DVPP_CTL_VPC_PROC, &dvppApiCtlMsg)) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[ImagePreProcessEngine] call dvppctl process:VPC faild");
    ...
    if(NULL == dvppOut_) {
        ...
        if (NULL == dvppOut_ || NULL == dvppOut_.get()) {
            HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[ImagePreProcessEngine] call dvppOut_ make_shared failed");
            HIAI_DVPP_DFree(outputConfigure->addr);
            outputConfigure->addr = nullptr;
            return HIAI_ERROR;
        }
    ...
    if (NULL == image || NULL == image.get()) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[ImagePreProcessEngine] call image make_shared failed");
        HIAI_DVPP_DFree(outputConfigure->addr);
        outputConfigure->addr = nullptr;
        return HIAI_ERROR;
    }
    ...
    try {
        ...
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[ImagePreProcessEngine] failed to allocate buffer for outBuffer");
        return HIAI_ERROR;
    }
    std::shared_ptr<u_int8_t> outBuffer(tmp);
    if (NULL == outBuffer || NULL == outBuffer.get()) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[ImagePreProcessEngine] call outBuffer shared_ptr failed");
        HIAI_DVPP_DFree(outputConfigure->addr);
        outputConfigure->addr = nullptr;
        return HIAI_ERROR;
    }
    ...
    outputConfigure->addr = nullptr;
    if (ret != 0) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[ImagePreProcessEngine] memcpy_s out buffer data error");
        outBuffer = nullptr;
        return HIAI_ERROR;
    }
...
int ImagePreProcessEngine::HandleDvpp()
{
    if (NULL == dvppIn_ || NULL == dvppIn_.get()) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[ImagePreProcessEngine]__hanlde_dvpp, input data is null!");
        return HIAI_ERROR;
    }
...
/**
* @ingroup hiaiengine
* @brief HIAI_DEFINE_PROCESS: realize ImagePreProcessEngine
* @[in]: define a input and output,
*        and register Engine named ImagePreProcessEngine
*/
HIAI_IMPL_ENGINE_PROCESS("ImagePreProcessEngine", ImagePreProcessEngine, INPUT_SIZE)
{
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[ImagePreProcessEngine] start process!");
    int errCode = HIAI_OK;
    ...
    if(arg0 != nullptr){
        ...
        if (!isSentinelImage(dataInput)) {
            if (dataInputIn_ != nullptr) {
                ...
                if (dataInputIn_ == nullptr){
                    HIAI_ENGINE_LOG(HIAI_IDE_WARNING, "[ImagePreProcessEngine] malloc error");
                    return HIAI_ERROR;
                }
            ...
            if (dataInputIn_->v_img.size() != dataInputIn_->b_info.batch_size) {
                HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[ImagePreProcessEngine] Wait for other %d batch image info!", (dataInputIn_->b_info.batch_size - dataInputIn_->v_img.size()));
                return HIAI_OK;
            }
        ...
        do{
            ...
            if (HIAI_OK != hiaiRet) {
                if (HIAI_ENGINE_NULL_POINTER == hiaiRet || HIAI_HDC_SEND_MSG_ERROR == hiaiRet || HIAI_HDC_SEND_ERROR == hiaiRet
                    ...
                    HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[ImagePreProcessEngine] SendData error[%d], break.", hiaiRet);
                    break;
                }
                HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[ImagePreProcessEngine] SendData return value[%d] not OK, sleep 200ms", hiaiRet);
                usleep(SEND_DATA_INTERVAL_MS);
            }
    ...
    if (HIAI_ERROR == errCode) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[ImagePreProcessEngine]dvpp process error!");
        goto ERROR;
    }
    ...
    ClearData();
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[ImagePreProcessEngine] end process!");
    // cout << "[ImagePreProcessEngine] end process!" << endl;
    return HIAI_OK;
