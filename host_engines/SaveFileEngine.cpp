...
#include <sstream>
#include <fcntl.h>
extern struct timeval stop1;
HIAI_StatusT SaveFileEngine::Init(const hiai::AIConfig& config, const  std::vector<hiai::AIModelDescription>& model_desc)
{
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[SaveFileEngine] start init!");
    ...
    if(realpath(info_file_.c_str(), path) == NULL){
        ...
        if(datainfo_path.substr(tmp_ind + 1) != "MnistDataset"){
            HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[SaveFileEngine] can not find %s!", postprocess_config_->info_file.c_str());
            return HIAI_ERROR;
        }
    }else{
    ...
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[SaveFileEngine] end init!");
    return HIAI_OK;
}

HIAI_IMPL_ENGINE_PROCESS("SaveFileEngine", SaveFileEngine, INPUT_SIZE)
{
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[SaveFileEngine] start process!");
    if (nullptr == arg0)
    {
    ...
    {
        HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[SaveFileEngine] sentinel image, process over.");
        ...
        do{
            ...
            if (HIAI_OK != hiai_ret) {
                if (HIAI_ENGINE_NULL_POINTER == hiai_ret || HIAI_HDC_SEND_MSG_ERROR == hiai_ret || HIAI_HDC_SEND_ERROR == hiai_ret
                    ...
                    HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[SaveFileEngine] SendData error[%d], break.", hiai_ret);
                    break;
                }
                HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[SaveFileEngine] SendData return value[%d] not OK, sleep 200ms", hiai_ret);
    ...
    for(unsigned int ind = 0; ind < tran->b_info.batch_size; ind++)
    {
        ...
        for(unsigned int i=0 ; i < output_data_vec.size(); ++i){
            ...
            if(size <= 0){
                HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[SaveFileEngine]: the OutPutT size less than 0!");
                return HIAI_ERROR;
            }
            ...
            try{
                ...
                HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[SaveFileEngine] alloc output data error!");
                return HIAI_ERROR;
            }
            int ret  = memcpy_s(result, sizeof(float)*size, out.data.get(), sizeof(float)*size);
            if(ret != 0){
                HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[SaveFileEngine] memcpy_s output data error!");
                delete[] result;
                result = NULL;
                return HIAI_ERROR;
            }
            ...
            if(fd == -1){
                HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[SaveFileEngine] open file %s error!", outFileName.c_str());
                delete[] result;
                result = NULL;
                return HIAI_ERROR;
            }
            ...
            for (int k = 0; k < oneResultSize; k++){
                ...
                if(ret == -1){
                    HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[SaveFileEngine] write file error!");
                    ret = close(fd);
                    if(ret == -1){
                        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[SaveFileEngine] close file error!");
                    }
                    delete[] result;
                    result = NULL;
                    return HIAI_ERROR;
    ...
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[SaveFileEngine] end process!");
    // cout << "[SaveFileEngine] end process!" << endl;
    // if (isSentinelImage(std::make_shared<BatchImageParaWithScaleT>(image_handle)))
    // {
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[SaveFileEngine] sentinel image, process over.");
    // std::shared_ptr<std::string> result_data(new std::string);
